...
namespace KalWeb.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Upload(HttpPostedFileBase fileUpload)
        {
           ...
           var ColAgentId= Session["userid"];
           //var ColAgentIdName = Session["lstAgentIdName"];
        ...
        {
            //Session.Clear();
            string lstAgentIdName = GetUserData();
            //string lstAgentIdName = "";
                            ...
                            if (file != null)
                            {
                                if (file.ContentType == "application/vnd.ms-excel" || file.ContentType == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                                {
                                    ...
                                    if (Extension == ".xlsx" || Extension == ".xls" || Extension == ".csv")
                                    {
                                        ...
                                        ValidExcel(dtbl, lstAgentIdName, columnNames, targetpath + "ExcelLogs_" + DateTime.Now.Day + '_' + DateTime.Now.Month + '_' + DateTime.Now.Year + ".xlsx");
        ...
        public DataTable ValidExcel(DataTable dt, string lstAgentIdName, string[] columnNames,string excelFilePath = null)
        {
...
//--------------------------------------------Excel Headers--------------------------------------------------//
            ...
            //var columncnt = 1;
                ...
                try
                {
                    string  agentsidname = lstAgentIdName;

                    string[] agentidname = agentsidname.Split(',');

                    Dictionary<int, string> agentData = new Dictionary<int, string>();

                    for (var i = 0; i < agentidname.Count() - 1; i++)
                    {
                        agentData.Add(Int32.Parse(agentidname[i].Split('~')[0]), agentidname[i].Split('~')[1].ToString().ToLower());
                    }
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        if (agentData.ContainsValue(Convert.ToString(dr["User id"]).ToLower()))
                        {
                            //Console.WriteLine("aa");
                        }else
                        {
                            WriteLog("User id not present for your account.", rowcnt, 37);
                            worksheet.Cells[rowcnt, 37].Style.Fill.PatternType = ExcelFillStyle.Solid;
                            worksheet.Cells[rowcnt, 37].Style.Fill.BackgroundColor.SetColor(Color.Red);
                            flag = false;
                        }
        ...
        public string GetUserData()
        {
            ...
            cmd.Parameters.AddWithValue("@MUserID", Session["userid"].ToString());
            cmd.Parameters.AddWithValue("@MAgencyID", Session["agencyid"].ToString());
        ...
        public static string DataTableToString(DataTable table)
        {
            ...
            return ss; //serializer.Serialize(list);
        }
